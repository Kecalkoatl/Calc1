...
public class ReadCalc {
    ...
    public void ReadCalc() {
        ...
        if (blocks.length < 0 || blocks.length > 3 ) {
            throw new IllegalArgumentException("Калькулятор может работать только с двумя числами!");
        }
        try {
            operation = blocks[1];
            ...
            throw new ArrayIndexOutOfBoundsException("Выражение введено неверно!");
        }
        try {
            if (flag) {
